...
if(isset($_POST['submit'])){
   ...
   //getting media
    ...
    $file_image = basename(escape($_FILES['image']['name']));
   ...
   if($select->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert = $conn->prepare("INSERT INTO `users`(name, email, password, image, updated_on) VALUES(?,?,?,?,?)");
         $insert->execute([$name, $email, $cpass, $image, $updatedon]);
         if($insert){
